...
class CheetahRepresenter():
    ...
    def represent(self, obs_seq, act_seq=None):
        obs = np.copy(obs_seq)
        mean_vx = np.array([obs[8, :].mean()])
        min_z = np.array([obs[0, :].min()])
...
class CMCRepresenter():
    ...
    def represent(self, obs_seq, act_seq):
        spent_energy = np.array([np.sum(act_seq[0, np.argwhere(~np.isnan(act_seq))] ** 2 * 0.1)])
        diff = np.array([np.nanmax(obs_seq[0, :]) - np.nanmin(obs_seq[0, :])])
        max = np.array([np.nanmax(obs_seq[0, :])])
...
class PendRepresenter():

    def __init__(self):
        self._description = ['mean height', 'spent energy']
        # define goal space
        self._initial_space= np.array([[-1,1], [0, 100]])  # space in which goal are sampled
        self._representation = None

    def represent(self, obs_seq, act_seq):
        spent_energy = np.array([np.sum(act_seq[0, :] ** 2 * 0.001)])
        mean_height = np.array([np.nanmean(np.cos(obs_seq[0, :]))])
        self._representation = np.concatenate([mean_height, spent_energy], axis=0)
